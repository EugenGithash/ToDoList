...
#def click(event):
 #  new_item_entry.configure(state=NORMAL)
  # new_item_entry.delete(0, END)
   #new_item_entry.unbind('<Button-1>', clicked)
...
Label(root, text='To-Do List', bg='purple', font=("Comic Sans MS", 15), wraplength=300).place(
    x=35, y=0)

tasks = Listbox(root, selectbackground='Gold', bg='Silver', font=('Comic Sans MS', 12), height=12, width=25)
...
add_btn = Button(root, text='Adăugare', bg='Green', width=10, font=('Helvetica', 12),
                 command=lambda: add_item(new_item_entry, tasks))
add_btn.place(x=45, y=350)
#clicked = add_btn.bind('<Button-1>', click)

delete_btn = Button(root, text='Ștergere', bg='Red', width=10, font=('Helvetica', 12),
